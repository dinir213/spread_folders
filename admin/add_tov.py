...
async def del_tov(message: types.Message, state: FSMContext):
    async with state.proxy() as data:
        ...
        if need_lvl == 3 and target_positions[0] == '/Delete':
            ...
            all_positions_str = f'Позиции в категории {target_category} и в подкатегории {target_subcategory} успешно удалены!'
...
def register_handlers_client(dp: Dispatcher):
    ...
    dp.register_callback_query_handler(choice_btn_in_tovs, text_startswith="del₢", state="*")
