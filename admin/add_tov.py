...
async def add_del_tov_main(call: types.CallbackQuery, state: FSMContext):
    await state.finish()
    try:
        await call.message.edit_caption(f"Выберите, что хотите сделать:\n", reply_markup=inline_kb_add_del_tov)
    except:
        ...
        with open(photo_url, 'rb') as photo_file:
            await bot.send_photo(photo=photo_file, chat_id=call.from_user.id,
                                 caption=f'Выберите, что хотите сделать:\n',
                                 reply_markup=inline_kb_add_del_tov)
    ...
    async with state.proxy() as data:
        ...
        if data['need_lvl'] == 1:
            if data['need_action'] == 'del':
                ...
                if inline_kb_all_categories[1] == 1:
                    await call.message.edit_caption(f"Выберете название категории:\n", reply_markup=inline_kb_all_categories[0])
                else:
                    await call.message.edit_caption(f"Вы пока не создали ни одну категорию товаров. Перед созданием подкатегории создайте категорию товаров\n", reply_markup=inline_kb_add_del_tov_back)

            elif data['need_action'] == 'add':
                await call.message.edit_caption(f"Введите название категории:\n", reply_markup=inline_kb_add_del_tov_back)
        ...
        if data['need_lvl'] == 2 or data['need_lvl'] == 3:
            ...
            if inline_kb_all_categories[1] == 1:
                ...
                await call.message.edit_caption(f"Вы пока не создали ни одну категорию товаров. Перед созданием/удалением подкатегории/позиции создайте категорию товаров\n", reply_markup=inline_kb_add_del_tov_back)
...
async def choice_btn_in_tovs(call: types.CallbackQuery, state: FSMContext):
    ...
    async with state.proxy() as data:
        ...
        print(f'Сейчас уровень{now_lvl}')

        if now_lvl == 1:
            if data['need_lvl'] == 1:
                await del_category_db(target_category)
                await call.message.edit_caption(f'Категория {target_category} успешно удалена Администратором {call.from_user.username} ✅', reply_markup=inline_kb_add_del_tov_back)
                ...
                if inline_kb_all_subcategories[1] == 1:
                    await call.message.edit_caption(f"Выберете название подкатегории:\n", reply_markup=inline_kb_all_subcategories[0])
                    await state.update_data({"now_lvl": 2})
                else:
                    await call.message.edit_caption(f"Подкатегорий товаров нету, вы можете создать из, вернувшись назад\n", reply_markup=inline_kb_add_del_tov_back)

        if now_lvl == 2:
            if data['need_lvl'] == 2:
                if data['need_action'] == 'add':
                    await call.message.edit_caption(f"Введите название подкатегории:\n", reply_markup=inline_kb_add_del_tov_back)
                    await add_del_category.target_category.set()
                elif data['need_action'] == 'del':
                    await del_subcategory_db(data['target_category'], click_btn[1])
                    await update_count_tovs_db(+1, 'category', data['target_category'], click_btn[1])
                    inline_kb_all_subcategories = await print_all_categories(
                        await view_all_subcategories_db(data['target_category']), 'subcategory')
                    if inline_kb_all_subcategories[1] == 1:
                        await call.message.edit_reply_markup(inline_kb_all_subcategories[0])
                    else:
                        await call.message.edit_caption(f"В данной категории товаров удалены все подкатегории\n", reply_markup=inline_kb_add_del_tov_back)

            elif data['need_lvl'] == 3:
                await state.update_data({"now_lvl": 3})
                if data['need_action'] == 'del':
                    await call.message.edit_caption(f"Введите номер позиции, который желаете удалить:\n")

        elif now_lvl == 3:
            if data['need_action'] == 'add':
                await call.message.edit_caption(f"Вводите позиции по три значения в одну позицию(Логин Ключ Резерв)\nЧтобы ввести много строк товара, пишите следующим образом:\n\nЛОГИН1 ПАРОЛЬ1 РЕЗЕРВ1 ЛОГИН2 ПАРОЛЬ2 РЕЗЕРВ2 ЛОГИН3 ПАРОЛЬ3 РЕЗЕРВ3\n", reply_markup=inline_kb_add_del_tov_back)
                await add_del_category.target_category.set()
            elif data['need_action'] == 'del':
                ...
                if len(all_positions_str) < 4095:
                    await call.message.edit_caption(all_positions_str, reply_markup=inline_kb_add_del_tov_back)
                else:
                    for x in range(0, len(all_positions_str), 4095):
                        await call.message.answer(all_positions_str[x:x + 4095])
