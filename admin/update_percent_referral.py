...
async def update_percent_referral(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        await bot.send_message(message.from_user.id, f'Процент от покупки рефераллов изменен на <b>{float(percent)}%</b> <i>администратором <b>{message.from_user.first_name}</b></i> ✅', reply_markup=inline_kb_back_in_menu, parse_mode='html')
        await state.finish()
    except:
