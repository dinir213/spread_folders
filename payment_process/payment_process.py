...
async def get_check(call: types.CallbackQuery):
    ...
    if isinstance(amount, float) and amount >= 0.01:
        if text[1] == 'yookassa':
            ...
            await call.message.edit_caption(f"Перейдите по ссылке для оплаты {amount} руб: {payment_url[0]}", reply_markup=(await kb_check_payment_buttons(text[1])))
            ...
            await call.message.edit_caption(f"Перейдите по ссылке для оплаты {amount} RUB: {paym_data_crypto[0]}", reply_markup=(await kb_check_payment_buttons(text[1])))
...
async def confirm_payment(call: types.CallbackQuery):
    ...
    if payment_method == 'yookassa':
        ...
        if payment_status == 'waiting_for_capture':
            try:
                await call.message.edit_caption('Для подтверждения платежа нажмите на кнопку "Оплачено" еще раз через 10 секунд', reply_markup=(await kb_check_payment_buttons(payment_method)))
            except:
                pass
        elif payment_status == 'pending':
            try:
                await call.message.edit_caption('Не удалось произвести платеж, создайте новый счет и попробуйте оплатить', reply_markup=(await kb_check_payment_buttons(payment_method)))
            except:
                pass
        elif payment_status == 'succeeded':
            await call.message.edit_caption(f'Ваш баланс был успешно пополнен на {amount} руб\n\nДобро пожаловать @{call.from_user.username}! Спасибо, что пользуетесь нашим магазином\n\nГлавное меню:', reply_markup=menu_kb.inline_kb_menu)
            ...
            await call.message.edit_caption(f'Вы отменили платеж\n Добро пожаловать @{call.from_user.username}! Спасибо, что пользуетесь нашим магазином\n\nГлавное меню:', reply_markup=menu_kb.inline_kb_menu)
            await del_payment_values_in_menu_payment(call)
        ...
        await del_payment_values_in_menu_payment(call)
        await call.message.edit_caption(f'Добро пожаловать, {call.from_user.first_name}! Спасибо, что выбрали наш магазин!\n\nГлавное меню:', reply_markup=menu_kb.inline_kb_menu)
